...
int main(){ 
    ...
    while (true) {
        ...
        while ( propagateGivesConflict(litToPropagate) ) {
            if ( decisionLevel == 0) { cout << "UNSATISFIABLE" << endl << "time : "<<(clock()-(float)t)/CLOCKS_PER_SEC<<endl; return 10; }
            backtrack();
            litToPropagate[0] = modelStack[modelStack.size()-1];
        }
        decisionLit= getNextDecisionLiteral();
        if (decisionLit == 0) { checkmodel(); cout << "SATISFIABLE" << endl<< "time : "<<(clock()-(float)t)/CLOCKS_PER_SEC<<endl; return 20; }
